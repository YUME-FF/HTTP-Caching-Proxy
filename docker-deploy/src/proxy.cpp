...
void Proxy::requestGET(int client_fd, int connection_fd, httpcommand request, int thread_id) {
    ...
    if (response_info.is_chunk) {
        ...
        int len = 0;
        while (recv_left > 0) {
            len = recv(client_fd, buffer + recv_first + recv_len, sizeof(buffer) - recv_first - recv_len, 0);
            recv_len += len;
            recv_left -= len;
        }
...
void Proxy::requestPOST(int client_fd, int connection_fd, httpcommand request_info, int thread_id) {
    ...
    size_t header_end = buffer_str.find("\r\n\r\n");
    int header_len = header_end + 4;
    int recv_left = response_info.content_length - (n - header_len);
    int len = 0;
    while (recv_left > 0) {
        len = recv(client_fd, buffer + n + recv_len, sizeof(buffer) - n - recv_len, 0);
        recv_len += len;
        recv_left -= len;
    }
